...
def lambda_handler(event, context):
    ...
    while True:
      try:
        ...
        ipv6_reqs.append(os.environ['Ipv6Req' + str(count + 1)])
        count = count + 1
      except KeyError:
        break
    ...
    if event["detail-type"] == "EC2 Instance-launch Lifecycle Action":
        index = 1
        for x in subnet_ids:
            interface_id = create_interface(x,secgroup_ids[index-1], ipv6_reqs[index-1])
...
def create_interface(subnet_id,sg_id,ipv6_reqs):
